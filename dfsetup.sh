...
install_Daggerfall_icon() {
 if [[ -d "$(xdg-user-dir DESKTOP)" ]]; then
  cp "TES_Daggerfall.desktop"  "$(xdg-user-dir DESKTOP)"/ && printf "Desktop shortcut created...\n";
  cp "dagger_icon.png" "${HOME}"/.config/Daggerfall/;
  printf "Exec=dosbox -conf %s/.config/Daggerfall/dagger.conf\n" "${HOME}" >> "$(xdg-user-dir DESKTOP)"/TES_Daggerfall.desktop;
  printf "Icon=%s/.config/Daggerfall/dagger_icon.png" "${HOME}" >> "$(xdg-user-dir DESKTOP)"/TES_Daggerfall.desktop;
  chmod 700 "$(xdg-user-dir DESKTOP)"/TES_Daggerfall.desktop
 else
