...
# 2) Unpack Daggerfall in $HOME/DOS (by default)
...
uninstall_Daggerfall() {
	printf "Are you sure you want to uninstall Daggerfall? All saved games and configuration files will be removed. [y/N] "; read -r answerYN
		case ${answerYN} in
			Y|y)
	...
	printf "Would you like a more modern WASD/Mouselook control scheme set up by default? [Y/n] "; read -r answerYN
		case ${answerYN} in
			Y|y|"")
				...
				sed -i 's/controls betaplyr.dat/controls viewplyr.dat/' "${Install_Dir}"/DAGGER/Z.CFG && printf "WASD/Mouselook control scheme applied\n"
				;;
			N|n)
				return
				;;
...
check_EUID;
printf "*** Daggerfall Installer for Linux ***\n\
Would you like to: \n\
